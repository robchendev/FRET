module.exports = {
  ...
  /**
   * Sets was thanked count since new update
   * @param {Callback} pointdata - the point data callback
   * @param {Number} numTimesThanks - an initial estimate of times thanked
   */
  initializeWasThanked: function (pointdata, numTimesThanked) {
    pointdata.wasThanked = numTimesThanked;
  },
  initializeGaveThanks: function (pointdata) {
    pointdata.gaveThanks = 0;
  },
  checkIfNeedInitialization: function (pointdata) {
    console.log(pointdata);
    if (!pointdata.wasThanked && !(pointdata.wasThanked > 0)) {
      this.initializeWasThanked(pointdata, Math.ceil(pointdata.points / 75));
    }
    if (!pointdata.gaveThanks && !(pointdata.gaveThanks > 0)) {
      this.initializeGaveThanks(pointdata);
    }
  ...
  createPointdata: function (
    ...
    const addPoints = new pointsAdd({
      userid: usersID,
      points: score,
      wasThanked: numTimesThanked,
      gaveThanks: numThanksGiven,
    });
